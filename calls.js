    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (let i = 0; i < (count); i++) {
                ...
                $(element).append(`<div class="contact" data-chat="${i}" id=${unread_chatlist[i].session_id}><img src="img/user.png" alt="openchat"><div class="name" }><h4 class="firstname" }>${unread_chatlist[i].sender}</h4><small class="message_time" }>${timestamp(unread_chatlist[i].timestamp)}</small><br><small class="sentence" }>${unread_chatlist[i].last_msg}</small></div></div>`);
